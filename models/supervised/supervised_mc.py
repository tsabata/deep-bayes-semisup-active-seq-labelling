...
class SupervisedModelAL(ALModel, nn.Module):
    ...
    def predict_mc(self, sents, device, T=1):
        torch.cuda.empty_cache()
        x = nn.utils.rnn.pack_sequence([sents[0] for _ in range(T)])
        ...
        out = self.forward(x)
        del x
        predictions = torch.argmax(out, dim=1).reshape(T, -1).cpu()
        del out

        return predictions

    def viterbi_paths(self, X: List[np.ndarray], device, repeats: int = 100):
        self.eval()
        ...
        with torch.no_grad():
            ...
            for batch_idx, x_batch in enumerate(self._data_generator(X)):
                predictions = self.predict_mc(x_batch, device, T=repeats)
                uniques, counts = np.unique(predictions, return_counts=True, axis=0)
                freqs = np.log(counts / repeats)
                sortinds = freqs.argsort()[::-1][:len(freqs)]
                paths.append(uniques[sortinds])
                path_probs.append(freqs[sortinds])
        return paths, path_probs

    def state_confidences(self, X: List[np.ndarray], device, repeats: int = 100, **kwargs) -> List[np.ndarray]:
        self.eval()
        for m in self.modules():
            if m.__class__.__name__.startswith('LSTM'):
                m.train()
        ...
        with torch.no_grad():
            state_confidences = []
            for batch_idx, x_batch in enumerate(self._data_generator(X)):
                occurences = np.zeros((x_batch[0].shape[0], self.n_classes))
                predictions = self.predict_mc(x_batch, device, T=repeats)
                for time in range(x_batch[0].shape[0]):
                    uniques, counts = np.unique(predictions[:, time], return_counts=True, axis=0)
                    for tag, count in zip(uniques, counts):
                        occurences[time, tag] = count
                normalised = (occurences.T / (occurences.sum(axis=1))).T
                state_confidences.append(np.nan_to_num(np.log(normalised)))
