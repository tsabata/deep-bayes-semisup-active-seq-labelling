...
if __name__ == "__main__":
    ...
    ID_sim_beta = 1
    dataset = Conll2003Dataset(save_file_path='datasets/saves/conll2003POS.pkl', task='POS')
    ...
    # ------------------------------------ create model ---------------------------------------
    dropout_rate = 0.5
    model = BilstmCRF(dataset.max_word_idx + 1, dataset.max_tag_idx + 1, 30, 100, 2, 0, 0.5, 0.5,
                      dataset.tag2idx(dataset.START_WORD), dataset.tag2idx(dataset.END_WORD),
                      True, dataset.word2vec(dataset.END_WORD), use_fc=False)
    ...
    for strategy in ['least_confident', 'token_entropy', 'random', 'least_confident',
        ...
        labeled_train_x, labeled_train_y = ltrain_dataset.x.copy(), ltrain_dataset.y.copy()
        unlabeled_train_x, unlabeled_train_y = utrain_dataset.x.copy(), utrain_dataset.y.copy()

        sim = copy.deepcopy(utrain_dataset.sim.tolist())

        test_dataset_x, test_dataset_y = test_dataset.x.copy(), test_dataset.y.copy()
